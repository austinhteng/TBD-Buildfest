...
if __name__ == '__main__':
    ...
    def text_detected(text):
        ...
        elif (len(full_sentences) > 0):
            rich_text = compare_with_book(full_sentences[-1])
    ...
    def remove_punctuation(txt):
        trg = txt.maketrans("", "", string.punctuation)
        return txt.translate(trg)

    def compare_with_book(sentence):
        global book_file, text_to_read
        
        sentence_pure = remove_punctuation(sentence).lower()
        text_to_read_pure = remove_punctuation(text_to_read).lower()

        if (sentence_pure == text_to_read_pure):
            text_to_read = book_file.readline().strip()
            # print(sentence_pure, text_to_read_pure)
            print(text_to_read)
            rich_text = Text(sentence, style="green")
        else:
            rich_text = Text(sentence, style="bold red")
        return rich_text
    ...
    recorder_config = {
        ...
        'no_log_file': True,
        'initial_prompt': (
            "Expect to receive sentances from the book The Lorax.\n"
